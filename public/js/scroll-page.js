...
function ScrollPage(config){
    ...
    this.config = config;
    this.defaultLoadingTemplate = "<div id='default_loading'  class='p_loading'><span >正在加载，请稍后...</span></div>";
    this.defaultFinishLoadingTemplate = "<div id='default_finish_loading'  class='p_loading'>没有更多了...</div>";
...
ScrollPage.prototype.init = function(){
    ...
    if(!this.config.before){//添加默认执行前的拦截器
        this.addBefore(function(){
            $("#default_loading").remove();//在用了默认加载样式的情况下
